...
void main() 
{
	...
	float opacity = albedo.a * colors.opacity.r * texture(opacityMap, fragUV).r;
	outAlbedo = vec4(albedo.rgb, opacity);

	outSpecular = colors.specular.r * texture(specularMap, fragUV).r;
}